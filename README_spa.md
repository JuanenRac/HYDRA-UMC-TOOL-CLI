...
* ✅ **`hydra-cli doctor [--server URL] [--config PATH]`** — diagnóstico de contrato del servidor en solo lectura: valida `/api/hydra-info` y `/api/settings` y comprueba que los totales publicados de controladores/robots coinciden con el listado. No envía comandos ni sondea hardware. *(implementado)*
