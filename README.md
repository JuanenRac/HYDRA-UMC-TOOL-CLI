...
* ✅ **`hydra-cli doctor [--server URL] [--config PATH]`** — read-only Server contract diagnosis: validates `/api/hydra-info` and `/api/settings`, then verifies published controller/robot totals against the settings roster. It never sends commands or probes hardware. *(implemented)*
