...
// afterward (dispatched through the exact same resolveTarget every
// one-shot command uses) inherits the config file's server and timeout
// without retyping --config on every line.
func TestCmdShell_SetsSessionServerAndTimeoutFromConfigFlag(t *testing.T) {
	for _, name := range []string{"HYDRA_CLI_SERVER", "HYDRA_CLI_TIMEOUT_SEC"} {
		original, hadOriginal := os.LookupEnv(name)
		defer func(name, original string, hadOriginal bool) {
			if hadOriginal {
				os.Setenv(name, original)
			} else {
				os.Unsetenv(name)
			}
		}(name, original, hadOriginal)
		os.Unsetenv(name)
	}

	path := writeConfigFile(t, `{"server": "http://config-session-target:9100", "timeoutSec": 9}`)
	fakeDispatch := func(args []string) ExitCode { return ExitOK }
	var out bytes.Buffer

	err := cmdShell(&out, strings.NewReader("quit\n"), []string{"--config", path}, fakeDispatch)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got := os.Getenv("HYDRA_CLI_SERVER"); got != "http://config-session-target:9100" {
		t.Fatalf("HYDRA_CLI_SERVER = %q, want the session --config file's own server", got)
	}
	if got := os.Getenv("HYDRA_CLI_TIMEOUT_SEC"); got != "9" {
		t.Fatalf("HYDRA_CLI_TIMEOUT_SEC = %q, want the session --config file's own timeoutSec", got)
	}
}
