...
* ✅ **`hydra-cli doctor [--server URL] [--config PATH]`** — diagnostica del contratto del server in sola lettura: convalida `/api/hydra-info` e `/api/settings` e confronta i totali pubblicati di controller/robot con l'elenco. Non invia comandi né sonda hardware. *(implementato)*
